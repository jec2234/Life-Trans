...
function clearPredictions() {
    var predictedTripList = document.querySelector('.predicted-trips').querySelectorAll('.trip-card-collection');
    for(var i = 0; i < predictionTrips.length; i++) {
        predictedTripList[0].remove();
    }
    predictionTrips = [];
    matrixTrips = [];
}
    ...
    if(status == 'OK') {
        ...
        for(var i = 0; i < durations.length; i++) {
            if(durations[i].status == "OK") {
                ...
                if(timeDifference > 0) {
                    matrixTrips[i].timeDiff = timeDifference;
                    validPredictions.push(matrixTrips[i]);
                }
            }
        }
        var sortedPredictions = validPredictions.sort(function(a,b) {
            var aTime = new Date(a.timeDiff);
            var bTime = new Date(b.timeDiff);
            if(aTime > bTime) {
                return 1;
            }
            if(aTime < bTime) {
...
function createPredictionRequest(origin) {
    ...
    for(var i = 0; i < uaTripListEles.length; i++) {
        ...
        if(!tripConfirmed) {
            ...
            var uaPuTime = new Date(uaTripListEles[i].querySelector('.pu-time-value').innerHTML);
            var originDoTime = new Date(origin.doTime);
...
function displayPredictions(sortedPredictions) {
    ...
    for(var i = 0; (i < sortedPredictions.length && i < 10); i++) {
        ...
        appChild.querySelector('.est-to-next').innerHTML = Math.ceil(sortedPredictions[i].timeDiff/60000) + " min";
