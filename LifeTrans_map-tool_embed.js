...
function generatePredictions() {
    if(routeTrips.length) {
        ...
        var originDOTimeObject = Date.parse(origin.doTime);
        var uaTripListEles = document.querySelector('.collection-list').querySelectorAll('.trip-card-collection'); //Needs to be replace with all trips in the cms
        for(var i = 0; i < uaTripListEles.length; i++) {
            if(origin.doTime) {
