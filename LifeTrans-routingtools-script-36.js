...
function disableButtons() {
    var clearRoute = document.querySelector('.clear-route-button');
    var confirmRoute = document.querySelector('.confirm-route-button');
    var generateRoute = document.querySelector('.generate-route-button');
    var generatePredictions = document.querySelector('.generate-predictions-button');
    var clearPredictions = document.querySelector('.clear-predictions-button');
    var addButtons = document.querySelector('.add-to-route');

    if(!clearRoute.classList.contains('disabled-button')) {
        clearRoute.classList.add('disabled-button');
    }
    if(!confirmRoute.classList.contains('disabled-button')) {
        confirmRoute.classList.add('disabled-button');
    }
    if(!generateRoute.classList.contains('disabled-button')) {
        generateRoute.classList.add('disabled-button');
    }
    if(!generatePredictions.classList.contains('disabled-button')) {
        generatePredictions.classList.add('disabled-button');
    }
    if(!clearPredictions.classList.contains('disabled-button')) {
        clearPredictions.classList.add('disabled-button');
    }
    if(!addButtons.classList.contains('disabled-button')) {
        addButtons.classList.add('disabled-button');
    }
}
...
function enableButtons() {
    ...
    if(clearRoute.classList.contains('disabled-button')) {
        clearRoute.classList.remove('disabled-button');
    }
    if(confirmRoute.classList.contains('disabled-button')) {
        confirmRoute.classList.remove('disabled-button');
    }
    if(generateRoute.classList.contains('disabled-button')) {
        generateRoute.classList.remove('disabled-button');
    }
    if(generatePredictions.classList.contains('disabled-button')) {
        generatePredictions.classList.remove('disabled-button');
    }
    if(clearPredictions.classList.contains('disabled-button')) {
        clearPredictions.classList.remove('disabled-button');
    }
    if(addButtons.classList.contains('disabled-button')) {
        addButtons.classList.remove('disabled-button');
    }
}
        ...
        if(!origin.doTime) {
            ...
            dirService.route(req, function(response, status) {
                if(status == 'OK') {
                    ...
                    createPredictionRequest(origin);
                    numRequests = requests.length;
                    distService.getDistanceMatrix(requests[numRequests - 1], function(resp, stat){
                        if(stat == 'OK') {
                            distanceCallback(resp, stat, origin, numRequests);
                        } else {
                            console.log("Distance Matrix Error: " + stat);
                            enableButtons();
                        }
                    });
                } else {
...
function createPredictionRequest(origin) {
        ...
        if(!tripConfirmed) {
            ...
            if(uaPuTime.getTime() > originDoTime.getTime() && uaPuTime.getTime() < (originDoTime.getTime() + 7200000)) {
                if(selectedLos.length == 0) {
                    destinationsArray.push(uaAddressString);
    ...
    var numRequests = Math.ceil(destinationsArray.length / 25);
    console.log("Requests: " + numRequests);
    for(var i = 0; i < numRequests; i++) {
        var req = {
            origins: [createAddress(origin, false)],
            destinations: destinationsArray.slice(i*25,(i+1)*25),
            travelMode: 'DRIVING',
            drivingOptions: {
...
function distanceCallback(response, status, origin, numRequests) {
    var origin = routeTrips[routeTrips.length - 1]; //Last trip
    if(status == 'OK') {
        numRequests = numRequests - 1;
        var currentMatrixTrips = matrixTrips.slice(numRequests*25, (numRequests + 1)*25);
        ...
        for(var i = 0; i < durations.length; i++) {
            ...
            if(durations[i].status == "OK") {
                ...
                var destinationPuTime = new Date(currentMatrixTrips[i].puTime);
                var timeDifference = (destinationPuTime.getTime() + lateBuffer) - (predictedArrivalTime.getTime() + losDelay);
                if(timeDifference > 0) {
                    currentMatrixTrips[i].predTimeMs = duration;
                    currentMatrixTrips[i].predTimeText = durationText;
                    validPredictions.push(currentMatrixTrips[i]);
                }
            }
        }
        if(numRequests == 0) {
            ...
            setTimeout(distService.getDistanceMatrix(requests[numRequests - 1], function(resp, stat){
                if(stat == 'OK') {
                    distanceCallback(resp, stat, origin, numRequests);
                } else {
                    console.log("Distance Matrix Error: " + stat);
...
function displayPredictions(sortedPredictions) {
    ...
    for(var i = 0; (i < sortedPredictions.length && i < 25); i++) {
        var clonedTrip = routeElement.cloneNode(true);
        ...
        appChild.querySelector('.do-time-value').innerHTML = sortedPredictions[i].doTime ? sortedPredictions[i].doTime : '';
