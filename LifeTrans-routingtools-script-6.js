    ...
    if(status == 'OK') {
        ...
        for(var i = 0; i < durations.length; i++) {
            ...
            if(durations[i].status == "OK") {
                ...
                var timeDifference = (destinationPuTime + lateBuffer) - (predictedArrivalTime + losDelay);
                if(timeDifference > 0) {
...
function getAirtableRecords(offset) {
    ...
    fetch(
        ...
        if(rawResponse.offset) {
            ...
            populateAllTripsArray(rawResponse.records);
...
function populateAllTripsArray(airtableJsonResponse) {
    for(var i = 0; i < airtableJsonResponse; i++) {
        var newTrip = {
            ...
            mileage: airtableJsonResponse[i].fields.Milage,
            comments: airtableJsonResponse[i].fields.DispatcheNotes,
            predTimeMs: 0,
            predTimeText: '',
        }
