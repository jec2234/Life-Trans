            ...
            dirService.route(req, function(response, status) {
                if(status == 'OK') {
                    ...
                    for(var i = 0; i < requests.length; i++) {
                        distService.getDistanceMatrix(requests[i], function(resp, stat){
                            if(stat == 'OK') {
...
function createPredictionRequest(origin) {
        ...
        if(!tripConfirmed) {
            ...
            if(uaPuTime.getTime() > originDoTime.getTime() && uaPuTime.getTime() < (originDoTime.getTime() + 7200000)) {
                if(selectedLos.length == 0) {
                    destinations.push(uaAddressString);
    ...
    var numRequests = Math.ceil(destinations.length / 25);
    for(var i = 0; i < numRequests; i++) {
        var req = {
            origins: [createAddress(origin, false)],
            destinations: destinations.slice(i*25,(i+1)*25),
            travelMode: 'DRIVING',
            drivingOptions: {
